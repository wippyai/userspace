...
local function handler(params)
    ...
    if not params.scouting_updates or type(params.scouting_updates) ~= "table" or next(params.scouting_updates) == nil then
        return { success = false, error = "Missing or invalid required parameter: scouting_updates (non-empty table)" }
    end
    ...
    updated_entry.data.scouting = updated_entry.data.scouting or {}

    -- Merge the updates
    deep_merge(updated_entry.data.scouting, params.scouting_updates)

    -- Validation after merge: Ensure model is present if scouting section exists
    if next(updated_entry.data.scouting) ~= nil and (not updated_entry.data.scouting.model or updated_entry.data.scouting.model == "") then
        return { success = false, error =
        "Invalid scouting config after update: 'model' cannot be empty or removed if the scouting section exists." }
    end
    ...
    if not final_entry_data then
        return {
            success = true,
            message = "Scouting config updated successfully, but failed to fetch final state.",
            entry_id = params.id,
    ...
    return {
        success = true,
        message = "Scouting config updated successfully.",
