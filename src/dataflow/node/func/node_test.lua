    ...
    describe("Function Node Integration Tests", function()
        ...
        describe("Control Command Tests", function()
            it("should execute control function and return child output", function()
                ...
                expect(output_content.message).to_equal("Control test")
                expect(output_content.processed_by).to_equal("test_function")
                expect(output_content.success).to_be_true()
            ...
            it("should execute chain workflow via control commands", function()
                ...
                -- Final output should be from the end of the chain
                expect(output_content.processed_by).to_equal("test_function")
                expect(output_content.success).to_be_true()
            ...
            it("should execute parallel workflow via control commands", function()
                ...
                expect(type(output_content)).to_equal("table")
                expect(#output_content).to_equal(2)

                -- Both should be successful test function results
                for _, branch_result in ipairs(output_content) do
                    if type(branch_result) == "string" then
                        local decoded, decode_err = json.decode(branch_result)
                        if not decode_err then
                            branch_result = decoded
                        end
                    end
                    expect(branch_result.processed_by).to_equal("test_function")
                    expect(branch_result.success).to_be_true()
                end
