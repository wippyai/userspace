...
-- Resolve the grant subject: a user_id or a group_id (exactly one).
...
local function handle(request_dto)
    ...
    local access_mask = request_dto.access_mask
    if type(access_mask) ~= "number" or access_mask < 0 then
        return { success = false, error = "access_mask must be a non-negative integer", kind = errors.INVALID }
    end
    ...
    -- ADMIN on the root authorizes granting the whole subtree.
    local has_access = ops.check_user_access(tx, user_id, root_id, ops.ACCESS.ADMIN)
    if not has_access then
        tx:rollback()
        db:release()
        return { success = false, error = "Insufficient access to grant on this subtree (ADMIN required)", kind = errors.PERMISSION_DENIED }
    end

    local result, err = ops.apply_subtree_access(tx, db, root_id, subject_id, access_mask :: integer, false)
    if err then
        tx:rollback()
        db:release()
