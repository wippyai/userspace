...
local function handle(request_dto)
    ...
    -- Validate additional access rules
    ...
    -- Component ID validation/generation
    ...
    local create_result, err_create = ops.handlers[ops.COMMAND_TYPES.CREATE_COMPONENT](tx, create_command)
    if err_create then
        tx:rollback()
        db:release()
        return { success = false, error = DATABASE_ERRORS.CREATE_FAILED .. ": " .. err_create }
    end

    -- Grant additional access permissions
    for _, access_rule in ipairs(additional_access) do
        ...
        local access_result, err_access = ops.handlers[ops.COMMAND_TYPES.GRANT_ACCESS](tx, access_command)
        if err_access then
            tx:rollback()
            db:release()
            return { success = false, error = DATABASE_ERRORS.ACCESS_FAILED .. ": " .. err_access }
        end
    end
