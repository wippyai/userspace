...
local function handle(request_dto)
    ...
    -- access_mask defaults to READ if not provided
    ...
    local reader = component_reader.new()
        ...
        :limit(2) -- need only to know if there are 0, 1, or 2+ results

    local components, read_err = reader:all()
    if read_err then
    ...
    local component = components[1] :: { component_id: string, private_context: any, access_level: integer?, impl_id: string }
